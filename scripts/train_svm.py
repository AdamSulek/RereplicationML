#!/usr/bin/env python3
"""Train a reproducible linear SVM baseline on a predefined scaffold split."""
...
def evaluate(model, features: np.ndarray, labels: np.ndarray) -> dict[str, float]:
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("artifacts/svm"))
    ...
    search = GridSearchCV(
        base_model,
        param_grid={"calibratedclassifiercv__estimator__C": [0.01, 0.1, 1.0]},
        scoring="roc_auc",
