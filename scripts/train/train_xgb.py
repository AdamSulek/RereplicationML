...
def metrics(labels: np.ndarray, probabilities: np.ndarray) -> dict[str, float | int]:
    """Preserve historical XGBoost keys while using the shared metric definitions."""
    result = screening_metrics(labels, probabilities)
    result["average_precision"] = result.pop("pr_auc")
    order = np.argsort(-probabilities, kind="stable")
    count = max(1, int(np.ceil(len(labels) * 0.10)))
    precision_at_10 = float(labels[order[:count]].mean())
    result["precision_at_10pct"] = precision_at_10
    prevalence = float(result["prevalence"])
    result["ef_at_10pct"] = precision_at_10 / prevalence if prevalence else 0.0
    result["cutoff_at_10pct"] = count
    return result
...
def run_scaffold(args: argparse.Namespace, data: dict[str, object], root: Path) -> dict[str, object]:
    ...
    selected_model.save_model(root / "model.json")
...
def run_kfold(args: argparse.Namespace, data: dict[str, object], params: dict[str, object], root: Path) -> list[dict[str, object]]:
    ...
    for fold in range(10):
        ...
        fitted.save_model(fold_dir / "model.json")
